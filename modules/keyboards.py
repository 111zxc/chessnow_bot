...
def matrix_to_keyboard(
    matrix: list, surr_button=True, reverse=False
        ...
        for i in range(8):
            for j in range(8):
                button = InlineKeyboardButton(
                    text=matrix[i][j], callback_data=chr(97 + j) + str(8 - i)
                )
    ...
    if reverse:
        for i in range(8):
            for j in range(8):
                button = InlineKeyboardButton(
                    text=matrix[7 - i][7 - j], callback_data=chr(104 - j) + str(i + 1)
                )
                buttons.append(button)
        keyboard.add(*buttons)

    if surr_button:
        surrender_button = InlineKeyboardButton(
