...
def main() -> None:
    ...
    def perform_move(move: str, session_id: str) -> Tuple[str, Optional[InlineKeyboardMarkup]]:
        ...
        current_session.side_to_move = 'белые' if current_session.side_to_move == 'чёрные' else 'чёрные'
        if current_board.is_checkmate():
            gif = generate_gif(current_board)
            ...
            if current_board.is_check():
                text += 'Шах!\n'
            keyboard = matrix_to_keyboard(fen_to_matrix(current_board.fen()), surr_button=True,
    ...
    async def callback_query_handler(query: types.CallbackQuery) -> None:
        ...
        current_session.add_player(user_id=query.from_user.id, user_name=query.from_user.first_name)
        ...
        await bot.edit_message_text(text=f'Белые: {current_session.player1_name}\n'
                                         f'Чёрные: {current_session.player2_name}\n'
                                         f'Ход: {move_count} ({current_session.side_to_move})\n',
                                    inline_message_id=query.inline_message_id)
        await bot.edit_message_reply_markup(inline_message_id=query.inline_message_id,
    ...
    async def callback_query_handler(query: types.CallbackQuery) -> None:
        ...
        :return: None
        """
        session = Session(player1_id=query.from_user.id,
                          player1_name=query.from_user.first_name,
                          session_id=query.inline_message_id)
        session_manager.add_session(session)
        await bot.edit_message_text(text=f'Белые: {query.from_user.first_name}\n'
                                         f'Чёрные: ожидаем',
                                    inline_message_id=query.inline_message_id)
        await bot.edit_message_reply_markup(inline_message_id=query.inline_message_id,
    ...
    async def callback_query_handler(query: types.CallbackQuery) -> None:
        ...
        current_session: Session = session_manager.find_session(query.inline_message_id)
        if query.from_user.id not in (current_session.player1_id, current_session.player2_id):
            pass
        else:
            board = current_session.board
            gif_url = generate_gif(board)
            move_count = board.fullmove_number
            surr_side = 'белые' if query.from_user.id == current_session.player1_id else 'чёрные'
    ...
    async def callback_query_handler(query: types.CallbackQuery) -> None:
        ...
        if (current_session.side_to_move == 'белые' and query.from_user.id == current_session.player1_id) \
                or (current_session.side_to_move == 'чёрные' and query.from_user.id == current_session.player2_id):
